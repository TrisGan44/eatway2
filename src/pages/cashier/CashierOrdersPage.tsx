...
import { getProducts, updateProduct, getProduct } from '../../api/produk'
...
export default function CashierOrdersPage() {
  ...
  const handleUpdateStatus = async (order: ApiOrder, status: string) => {
    ...
    try {
      if (status === 'Diproses') {
        // Kurangi stok berdasarkan detail pesanan
        const detailRes = await getOrderDetails()
        const details = Array.isArray(detailRes?.data) ? detailRes.data : Array.isArray(detailRes) ? detailRes : []
        const targetDetails = (details as ApiOrderDetail[]).filter((d) => d.pesanan_id === order.pesanan_id)
  ...
  return (
    <div className="cashier-page">
      ...
      <CashierOrderModal
        ...
        secondaryLabel="Tolak"
        onPrimary={() => selectedOrder && handleUpdateStatus(selectedOrder, 'Diproses')}
        onSecondary={() => selectedOrder && handleUpdateStatus(selectedOrder, 'Dibatalkan')}
      />
    </div>
  )
}
